...
```{.molviewspec title="Colored Protein Representation" height="700px"}
const structure = builder
  .download({ url: 'https://www.ebi.ac.uk/pdbe/entry-files/1cbs.bcif' })
  .parse({ format: 'bcif' })
  .modelStructure();
...
```{.molviewspec title="Protein with Ligand" height="700px"}
const structure = builder
  .download({ url: 'https://www.ebi.ac.uk/pdbe/entry-files/1cbs.bcif' })
  .parse({ format: 'bcif' })
  .modelStructure();
...
```{.molviewspec height="800px" width="100%" title="Multiple Representations"}
const structure = builder
  .download({ url: 'https://www.ebi.ac.uk/pdbe/entry-files/1cbs.bcif' })
  .parse({ format: 'bcif' })
  .modelStructure();
...
## Multiple Structures in One Document

You can have multiple MolViewSpec blocks in the same document:

```{.molviewspec title="First Structure"}
const structure = builder
  .download({ url: 'https://www.ebi.ac.uk/pdbe/entry-files/1cbs.bcif' })
  .parse({ format: 'bcif' })
  .modelStructure();
...
```{.molviewspec title="Second Structure"}
const structure = builder
  .download({ url: 'https://www.ebi.ac.uk/pdbe/entry-files/1cbs.bcif' })
  .parse({ format: 'bcif' })
  .modelStructure();
...
- **height**: Set the height of the viewer (default: "600px")
...
- `color()`: Apply colors (named colors, hex codes, or color schemes like 'rainbow')
